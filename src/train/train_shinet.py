...
from src.models.shinet import ShinetBiLSTM
...
# LOAD THE DATASET. 
train_dataset = np.load("data/trainset_hcs_augmented_combined.npy")

np.random.shuffle(train_dataset)

train_dataset = [(name, label) for name, label in train_dataset]
val_dataset = np.load("data/valset_hcs.npy")
...
# Create our model
model2network = {
    "cnn_owen": ShinetBiLSTM(num_classes=num_classes, hidden_size=hidden_size)
}
...
model_path = "results/shinet"

pretrained_dict = torch.load(model_path + "/shinet_24epoch_0_06890_cer_0_195_wer.pt")
...
for name, param in network.named_parameters():
    if "conv5" in name or "conv6" in name:
        param.requires_grad = True
"""
...
earlyStopping = EarlyStopping(monitor="val_CER", patience=12, mode="min", verbose=1)
...
reduce_lr = ReduceLROnPlateau(monitor="val_CER", factor=0.9, patience=5, verbose=1, mode="min", min_lr=1e-6)
...
wandbLogger = WandbLogger(
    project="shinet-finetuning",
    name="hcs-big-data-combine-augmented-head-network-fifty-epoch",
...
model.fit(
    ...
    callbacks=[ modelCheckpoint, wandbLogger, earlyStopping]
)
