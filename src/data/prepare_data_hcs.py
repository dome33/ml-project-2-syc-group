...
if __name__ == "__main__":
    ...
    print(f"Testset from chess reader: {len(chess_reader_dataset[:split])}") 


    n_total = len(online_dataset)
    n_train = int(n_total * ONLINE_TRAIN)
    n_val = int(n_total * ONLINE_VAL)

    train_data = online_dataset[:n_train]
    val_data = online_dataset[n_train:n_train + n_val]
    test_data = online_dataset[n_train + n_val:]

    test_data += chess_reader_dataset[:split]
    
    random.shuffle(val_data)

    print(f"Valset: {len(val_data)}") 
    np.save("data/valset_hcs.npy", val_data) 
    
    
    random.shuffle(train_data) 
    np.save("data/trainset_hcs.npy", train_data) 
    
    print(f"Trainset: {len(train_data)}")

    np.save("data/testset_hcs.npy", test_data) 

    print(f"Testset: {len(test_data)}") 
